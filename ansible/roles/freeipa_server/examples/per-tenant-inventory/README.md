...
run**. Point it at your IPA primary, supply one admin credential, and run `--tags iam`.
...
└── tenants/                      # ← freeipa_iam_tenants_dir points here
...
`freeipa_iam_tenants_dir` (set in `realm.yml` to `{{ inventory_dir }}/tenants`) makes the role
read **every** `tenants/*.yml`, flatten them into the native `freeipa_iam_*` lists, and stamp
...
exactly as `--tags export` emits them** (`freeipa_iam_usergroups`, `freeipa_iam_users`,
`freeipa_iam_hbac_rules`, `freeipa_server_dns_records`, …) with the export's entry fields
...
- **`bg-emergency`** (`freeipa_iam_breakglass_accounts`) — login stays enabled, auto-added
  ...
  `freeipa_iam_nologin_accounts: [admin]` to take admin's shell away (its API/kinit keep
  working — the role refuses this lockdown until a break-glass account exists, and always
  refuses fully *disabling* admin).
- **`svc-freeipa-automation`** (`freeipa_iam_service_accounts`) — a nologin automation
...
- **`globex.yml`** — fully header-driven. Every tenant/env-derived name, description, automember
  ...
  shows `freeipa_iam_sudo_commands: "{{ shared_sudo_commands }}"` pulling a whole list **in
...
`freeipa_iam_reconcile_scope: "<tenant>-<env>-"` later matches on.
...
Both resolve because validation runs on the **realm-wide merged** `freeipa_iam_usergroups` /
`freeipa_iam_users` — a role's `member_of` and `members` may reference anything declared
...
Share the identity by pointing both inventories' `freeipa_iam_tenants_dir` at one common
...
ansible-playbook -i inventories/qa/inventory.yml site.yml --tags iam \
  -e freeipa_server_authoritative=true -e freeipa_iam_reconcile_scope=acme- --check --diff
ansible-playbook -i inventories/qa/inventory.yml site.yml --tags iam \
  -e freeipa_server_authoritative=true -e freeipa_iam_reconcile_scope=acme-

# 2) Confident? Same command, live inventory:
ansible-playbook -i inventories/live/inventory.yml site.yml --tags iam \
  -e freeipa_server_authoritative=true -e freeipa_iam_reconcile_scope=acme-
```
