...
| [`image-mirror/`](image-mirror/) | The CI half of that pipeline, reusable on its own: pin an upstream release, resolve its image list, fetch every image into one OCI layout, SBOM and scan each one, decide a ratchet gate against the last published release, then publish, sign, and pack a transfer archive. Vendor-neutral — registry, object store, proxy, CA and signing key are all variables. |
...
| [Image mirror pipeline](image-mirror/) | Pin an upstream release in one file, let a bot open the bump MR, and have CI resolve the release's image list, fetch every image into a single OCI layout, build a CycloneDX SBOM and a vulnerability report per image, and gate on a **ratchet** — fail only on a fixable finding the last published release did not already carry, because on vendor images a fixed-severity gate blocks every release forever. Then push, `cosign sign` and attest against the pushed digest, and pack one transfer archive for an air-gapped destination, leaving out blobs the far side says it already has. Every host, credential, CA and key is a variable; the traps (a stale scanner that writes empty reports, signatures that never verify without a transparency log, a proxy that only breaks for some clients) are documented with the fix. | `image-mirror/` |
