...
| [`quay-layer-transfer/`](quay-layer-transfer/) | Container images across a one-way link, one new layer at a time: a low-side and a high-side Quay, NiFi send/receive flows that deduplicate layers against a Redis ledger, an OCI store on the far side, and the export/import/verify scripts. One compose file, proven with three transfers (22 blobs, then 6, then 0). |
...
| [Quay layer transfer](quay-layer-transfer/) | Operating procedure for moving N, N-1, N-2 of a semver image group across a diode: `export.sh` builds one OCI layout of the newest versions, a NiFi flow drops every blob whose digest Redis has seen and repacks the rest, the receive flow unpacks into a persistent OCI store, `import.sh` merges and `skopeo copy`s into the high Quay, `verify.sh` pulls and compares digests. Lab is two Quay 3.15 instances plus NiFi 2.5 in one compose file. | `quay-layer-transfer/` |
