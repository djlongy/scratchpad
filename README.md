...
| [`docs-as-code/`](docs-as-code/) | Documentation kept as Markdown in git, reviewed like code and published by CI. Zensical / Material for MkDocs tutorial with a runnable example site, a GitLab Pages setup guide, a CI job that mirrors `docs/` into the GitLab wiki for instances without Pages, and a container build plus Kubernetes deploy for teams on RKE2. |
