...
| [GitLab wiki mirror](docs-as-code/gitlab-wiki-mirror/) | `docs/` stays the source; a CI job clones the project wiki, rewrites it from the repo (index pages become section pages, links rewritten to wiki paths, `_sidebar.md` from the `.pages` nav), refuses to overwrite hand edits, and pushes only on change. For GitLab instances without Pages. | `docs-as-code/` |
